...
def kb_detail(request, slug):
    ...
    if request.user.role == 'customer' and not article.is_public:
        ...
        return HttpResponseForbidden(_("Sie haben keine Berechtigung, diesen Artikel zu sehen."))
    ...
    if request.method == 'POST':
        ...
        if vote == 'helpful':
            ...
            messages.success(request, _("Vielen Dank für Ihr Feedback!"))
...
def kb_create(request):
    ...
    if request.user.role == 'customer':
        ...
        return HttpResponseForbidden(_("Keine Berechtigung"))



    if request.user.role == 'support_agent' and (not request.user.support_level or request.user.support_level < 2):

        messages.error(request, _("Nur Support Agents Level 2 und höher können Artikel erstellen."))
    ...
    if request.method == 'POST':
        ...
        messages.success(request, _("Artikel \"%(title)s\" wurde erstellt!") % {"title": article.title})
...
def kb_edit(request, slug):
    ...
    if request.user.role == 'support_agent':
        ...
        if article.author != request.user and (not request.user.support_level or request.user.support_level < 2):
            ...
            messages.error(request, _("Sie können nur Ihre eigenen Artikel bearbeiten oder benötigen Level 2+."))
    ...
    if request.method == 'POST':
        ...
        messages.success(request, _("Artikel \"%(title)s\" wurde aktualisiert!") % {"title": article.title})
