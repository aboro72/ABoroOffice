...
def admin_settings(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            messages.success(request, _("Einstellungen wurden erfolgreich gespeichert!"))
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            messages.success(
                ...
                _("Lizenz erfolgreich aktiviert! Produkt: %(product)s, Gültig bis: %(expiry)s")



                % {"product": license_info.get("product_name"), "expiry": license_info.get("expiry_date")}
...
def user_create(request):
    ...
    if request.method == 'POST':
        ...
        if not username:
            ...
            errors.append(_("Benutzername ist erforderlich"))
            ...
            errors.append(_("Benutzername bereits vergeben"))
        ...
        if not email:
            ...
            errors.append(_("E-Mail ist erforderlich"))
            ...
            errors.append(_("E-Mail bereits vergeben"))
        ...
        if not first_name:
            ...
            errors.append(_("Vorname ist erforderlich"))
        ...
        if not last_name:
            ...
            errors.append(_("Nachname ist erforderlich"))
        ...
        if not password or len(password) < 6:
            ...
            errors.append(_("Passwort muss mindestens 6 Zeichen lang sein"))
        ...
        if errors:
            ...
            try:
                ...
                messages.success(request, _("Benutzer \"%(username)s\" wurde erfolgreich erstellt!") % {"username": username})
                ...
                messages.error(request, _("Fehler beim Erstellen des Benutzers: %(error)s") % {"error": str(e)})
...
def user_edit(request, user_id):
    ...
    if request.method == 'POST':
        ...
        if new_password and len(new_password) < 6:
            ...
            errors.append(_("Neues Passwort muss mindestens 6 Zeichen lang sein"))
        ...
        if errors:
            ...
            try:
                ...
                messages.success(request, _("Benutzer \"%(username)s\" wurde erfolgreich aktualisiert!") % {"username": username})
                ...
                messages.error(request, _("Fehler beim Aktualisieren des Benutzers: %(error)s") % {"error": str(e)})
...
def user_delete(request, user_id):
    ...
    if user_obj.id == request.user.id:
        ...
        messages.error(request, _("Sie können sich nicht selbst löschen!"))
    ...
    if request.method == 'POST':
        ...
        try:
            ...
            messages.success(request, _("Benutzer \"%(username)s\" wurde erfolgreich gelöscht!") % {"username": username})
            ...
            messages.error(request, _("Fehler beim Löschen des Benutzers: %(error)s") % {"error": str(e)})
...
def user_toggle_active(request, user_id):
    ...
    try:
        ...
        return JsonResponse({
            ...
            'message': _("Benutzer %(status)s") % {"status": _("aktiviert") if user_obj.is_active else _("deaktiviert")}
...
def retro_game(request):



    """Secret retro runner game - Easter egg for logged in users"""



    return render(request, 'games/retro_runner.html')
...
def virus_kong_game(request):



    """Virus Kong game - Donkey Kong variant"""



    return render(request, 'games/virus_kong.html')
...
def game_save_score(request):



    """Save game score (AJAX)"""
    ...
    try:
        ...
        rank = GameHighscore.objects.filter(score__gt=score).count() + 1
...
def game_get_highscores(request):
    ...
    try:
        ...
        scores_data = [{
            ...
            'player_name': score.player.full_name if hasattr(score.player, 'full_name') else f"{score.player.first_name} {score.player.last_name}",
            ...
            'is_current_user': score.player.id == request.user.id
        ...
        player_best = GameHighscore.get_player_best_score(request.user)



        player_best_data = None
        ...
        if player_best:
            ...
            player_rank = GameHighscore.objects.filter(score__gt=player_best.score).count() + 1
            ...
            player_best_data = {
                ...
                'level': player_best.level_reached,



                'coins': player_best.coins_collected,



                'date': player_best.created_at.strftime('%d.%m.%Y %H:%M')
        ...
        return JsonResponse({
            ...
            'player_best': player_best_data,



            'total_players': GameHighscore.objects.values('player').distinct().count()
...
def game_save_score_viruskong(request):



    """Save Virus Kong score (AJAX)"""
    ...
    try:
        ...
        rank = VirusKongHighscore.objects.filter(score__gt=score).count() + 1
...
def game_get_highscores_viruskong(request):
    ...
    try:
        ...
        player_best = VirusKongHighscore.get_player_best_score(request.user)
        ...
        if player_best:
            ...
            player_rank = VirusKongHighscore.objects.filter(score__gt=player_best.score).count() + 1
        ...
        return JsonResponse({
            ...
            'total_players': VirusKongHighscore.objects.values('player').distinct().count()
